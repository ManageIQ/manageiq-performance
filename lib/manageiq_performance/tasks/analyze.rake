namespace :manageiq_performance do
  begin
    ...
    task :stackprof_analyze, [:dir, :method] do |t, args|
      ...
      require "manageiq_performance/commands/analyze"
      opts =  [args[:dir]]
    ...
    task :stackprof_first, [:method, :route_index] do |t, args|
      Rake::Task["manageiq_performance:stackprof_analyze"].invoke "--first", args[:method]
    end

    desc "Analyze, with stackprof, a route in the last suite"
    task :stackprof_last, [:method, :route_index] do |t, args|
      Rake::Task["manageiq_performance:stackprof_analyze"].invoke "--last", args[:method]
    end
